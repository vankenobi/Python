...
class MainApp(QMainWindow , ui):
    ...
    #def mail_gonder(self):
        ...
        #for i in range(satir_sayisi):
            #ogrenci_tc = self.tableWidget.item(i,5)
            #teslim_tarihi = self.tableWidget.item(i,7)
            #bugun = datetime.now()
            
            #teslim_tarihi = datetime.strptime()
            ...
            #                                database = "postgres")
            #self.cursor = self.connection.cursor()
            #self.cursor.execute("""SELECT ogrenci_email FROM ogrenciler WHERE ogrenci_tc=%s""",(ogrenci_tc))
